...
const WeatherTestHarness = () => {
  ...
  return (
    <Container className="mt-5">
      ...
      {results && (
        <>
          <Alert variant={results.anomalies.length === 0 ? 'success' : 'warning'}>
            <h5>Test Results</h5>
            ...
            <p><strong>Anomalies:</strong> {results.anomalies.length.toLocaleString()}</p>
          ...
          {results.anomalies.length > 0 && (
            <Card>
              <Card.Header>
                <h5>Anomalies Detected ({results.anomalies.length})</h5>
              </Card.Header>
              <Card.Body style={{ maxHeight: '400px', overflowY: 'auto' }}>
                <Table striped bordered hover size="sm">
                  <thead>
