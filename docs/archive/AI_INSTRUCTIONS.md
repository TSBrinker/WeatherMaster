...
### Test Harness Integration (IMPORTANT!)

**When implementing new weather features, always add testing to the test harness.**

The test harness (`?test=true`) is the primary validation tool for weather generation. When adding new features:

1. **Add tracking stats** in the `stats` object initialization
2. **Initialize per-biome trackers** in the biome loop setup
3. **Collect data** during the test loop (typically at noon to avoid duplicates)
4. **Display results** in a new Card section in the UI
5. **Include in exports** - add to `downloadProblemsOnly()` for debugging

Example pattern (from Environmental Conditions):
```javascript
// 1. Add to stats initialization
environmentalStats: {}

// 2. Initialize per biome
stats.environmentalStats[biomeName] = { droughtDays: {...}, ... }

// 3. Collect in test loop (at noon)
if (hour === 12) {
  const conditions = service.getConditions(region, date);
  // Track levels and counts
}

// 4. Add UI Card to display results
// 5. Include in problemsReport for export
```

This ensures all weather features can be validated across all 43 biomes over a full simulated year.
