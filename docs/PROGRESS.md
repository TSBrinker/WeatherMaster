...
**Environmental Conditions** (cumulative states, not instant events) - **PHASE A COMPLETE** ✅
- [x] **Drought Detection & Display** *(Sprint 15 - Hemlock)*
  - Track rolling 30-day precipitation vs expected for biome/season
  - Severity levels: Normal → Abnormally Dry → Moderate → Severe → Extreme
  - Gameplay impacts: water scarcity, fire danger, crop stress
- [x] **Flooding Conditions** *(Sprint 15 - Hemlock)*
  - Track cumulative excess precipitation (14-day lookback)
  - Heavy precipitation factor included
  - Severity levels: Normal → Elevated → Moderate → High
- [x] **Heat Waves / Cold Snaps** *(Sprint 15 - Hemlock)*
  - Consecutive days above/below threshold temperatures (10°F+ deviation)
  - Severity levels: Normal → Advisory (3+ days) → Warning (5+ days) → Extreme (7+ days)
  - Gameplay impacts for prolonged exposure
- [x] **Wildfire Risk** *(Sprint 15 - Hemlock)*
  - Composite of drought + heat + humidity + wind
  - Recent rain reduces risk
  - Severity levels: Low → Moderate → High → Very High → Extreme
- [x] **UI Integration** *(Sprint 15 - Hemlock)*
  - Badge on PrimaryDisplay when alerts active
  - Modal with detailed breakdown and gameplay impacts
  - Test harness tracking for all environmental conditions

**Extreme Weather Events** (rare, impactful occurrences) - PHASE C
...
**Snow & Ice Tracking** - PHASE B **COMPLETE** ✅
- [x] Snow accumulation (inches on ground, melting rate) *(Sprint 16 - Alder)*
- [x] Ice accumulation from freezing rain *(Sprint 16 - Alder)*
- [x] Ground conditions (frozen, thawing, muddy, dry) *(Sprint 16 - Alder)*
- [x] Visual snow fill effect on PrimaryDisplay *(Sprint 16 - Alder)*
- [x] Snow toggle preference *(Sprint 16 - Alder)*
- [x] Test harness integration with snow stats *(Sprint 16 - Alder)*
...
- [ ] CSV export option
- [ ] Per-table export option (focused data for AI analysis)
- [ ] Modularize test harness into smaller files (file is large)
- [ ] Multi-year testing option (vary year seed for different weather scenarios)
...
**Project Status**: Sprint 16 Complete - Snow & Ice Accumulation → System Stable 🌲
